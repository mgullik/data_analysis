...
program cross
  ...
  real   , allocatable :: rc_freq(:,:), ic_freq(:,:)
  real   , allocatable :: pw_freq1(:,:), pw_freq2(:,:)
  real   , allocatable :: pw_freq1_ave(:), pw_freq2_ave(:), &
                          pw2_freq1_ave(:), pw2_freq2_ave(:)
  real   , allocatable :: rc_freq_ave(:), ic_freq_ave(:), rc2_freq_ave(:), ic2_freq_ave(:), rc_ic_freq_ave(:)

  real   , allocatable :: lag_freq(:), std_rc_freq_ave(:), std_ic_freq_ave(:), deriv_rc(:), deriv_ic(:), error_prop(:), coher2_freq(:), err_cohe_lag(:), covariance(:), errA_rc_ic(:), errA_lag(:)
...
!GET LIGHT CURVEs 
  ...
  write(*,*)
   ...
   write(*,*)
   name_base = 'cross_spec_real_vs_freq.dat'  
   ...
   name_base = 'cross_spec_imaginary_vs_freq.dat'  
...
!Errors through standard deviation on real and imaginary part      
      ...
      ! covariance(jj) = 0.0 
      deriv_rc(jj) =  (-1. *  ic_freq_ave(jj) / rc_freq_ave(jj)**2) / (1 + (ic_freq_ave(jj) / rc_freq_ave(jj))**2 )
      deriv_ic(jj) =                      (1. / rc_freq_ave(jj))    / (1 + (ic_freq_ave(jj) / rc_freq_ave(jj))**2 )
      error_prop(jj) = sqrt( (deriv_rc(jj)**2 * std_rc_freq_ave(jj)**2) + (deriv_ic(jj)**2 * std_ic_freq_ave(jj)**2) + (2. * deriv_rc(jj) * deriv_ic(jj) * covariance(jj)) ) / (2. * pi * freq(jj))
...
!Adam's formula lag
      ...
      lag_freq(jj) = atan2(ic_freq_ave(jj), rc_freq_ave(jj)) / (2 * pi * freq(jj))
