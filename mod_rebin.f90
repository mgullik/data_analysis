...
!Adam's formula lag
  ...
  subroutine rebin_lag_freq_obs(freq, rc_freq_obs, ic_freq_obs, pw_freq1_obs, pw_freq2_obs, int_number_max, int_number_obs, int_len_dim, obs_num, reb_dim)
    implicit none

    integer         , intent(IN)  :: obs_num, int_number_max, int_len_dim, int_number_obs(obs_num)
    double precision, intent(IN)  :: freq(int_len_dim), rc_freq_obs(int_number_max, int_len_dim / 2, obs_num), ic_freq_obs(int_number_max, int_len_dim / 2, obs_num), &
         pw_freq1_obs(int_number_max, int_len_dim / 2, obs_num), pw_freq2_obs(int_number_max, int_len_dim / 2, obs_num)

    integer              :: i, j, k, o, reb_dim, start, end_reb, tot_interval_reb
    double precision     :: reb_fac, bias2
    integer, allocatable :: reb_array(:)
    ...
    do o = 1, obs_num
       do i = 1, int_number_obs(o)
          ...
          do j = 1, reb_dim
             ...
             do k = start, end_reb
                rc_freq_reb   (j) = rc_freq_reb   (j) + rc_freq_obs(i, k, o)
                ic_freq_reb   (j) = ic_freq_reb   (j) + ic_freq_obs(i, k, o)
                rc2_freq_reb  (j) = rc2_freq_reb  (j) + (rc_freq_obs(i, k, o) * rc_freq_obs(i, k, o))
                ic2_freq_reb  (j) = ic2_freq_reb  (j) + (ic_freq_obs(i, k, o) * ic_freq_obs(i, k, o))
                rc_ic_freq_reb(j) = rc_ic_freq_reb(j) + (rc_freq_obs(i, k, o) * ic_freq_obs(i, k, o))
                pw_freq1_reb  (j) = pw_freq1_reb  (j) + pw_freq1_obs(i, k, o)
                pw2_freq1_reb (j) = pw2_freq1_reb (j) + (pw_freq1_obs(i, k, o) * pw_freq1_obs(i, k, o))
                pw_freq2_reb  (j) = pw_freq2_reb  (j) +  pw_freq2_obs(i, k, o)
                pw2_freq2_reb (j) = pw2_freq2_reb (j) + (pw_freq2_obs(i, k, o) * pw_freq2_obs(i, k, o))
             enddo
          enddo
       enddo
       tot_interval_reb = tot_interval_reb + int_number_obs(o)
    enddo
    
    do j = 1, reb_dim - 1
       rc_freq_reb   (j) = rc_freq_reb   (j) / dble(tot_interval_reb * reb_array(j))
       ic_freq_reb   (j) = ic_freq_reb   (j) / dble(tot_interval_reb * reb_array(j))
       rc2_freq_reb  (j) = rc2_freq_reb  (j) / dble(tot_interval_reb * reb_array(j))
       ic2_freq_reb  (j) = ic2_freq_reb  (j) / dble(tot_interval_reb * reb_array(j))
       rc_ic_freq_reb(j) = rc_ic_freq_reb(j) / dble(tot_interval_reb * reb_array(j))
       pw_freq1_reb  (j) = pw_freq1_reb  (j) / dble(tot_interval_reb * reb_array(j))
       pw_freq2_reb  (j) = pw_freq2_reb  (j) / dble(tot_interval_reb * reb_array(j))
       pw2_freq1_reb (j) = pw2_freq1_reb (j) / dble(tot_interval_reb * reb_array(j))
       pw2_freq2_reb (j) = pw2_freq2_reb (j) / dble(tot_interval_reb * reb_array(j))
    enddo
...
!Errors through standard deviation on real and imaginary part      
      !lag with propagation formula
      std_rc_freq_reb(j) = sqrt((rc2_freq_reb(j) - (rc_freq_reb(j) * rc_freq_reb(j)) ))  / (sqrt(dble(tot_interval_reb * reb_array(j))))
      std_ic_freq_reb(j) = sqrt((ic2_freq_reb(j) - (ic_freq_reb(j) * ic_freq_reb(j)) ))  / (sqrt(dble(tot_interval_reb * reb_array(j))))
      reb_covariance(j)  = (rc_ic_freq_reb(j) - ((rc_freq_reb(j) * ic_freq_reb(j) ) ) )  /       dble(tot_interval_reb * reb_array(j))
      ! covariance(j) = 0.0 
      deriv_rc(j) =  (-1.d0 *  ic_freq_reb(j) / rc_freq_reb(j)**2) / (1 + (ic_freq_reb(j) / rc_freq_reb(j))**2 )
      deriv_ic(j) =                      (1.d0 / rc_freq_reb(j))    / (1 + (ic_freq_reb(j) / rc_freq_reb(j))**2 )
...
!Coherence and error on the lag with the coherence formula
      ...
      reb_err_cohe_lag(j) = sqrt( (1.d0 - reb_coher2_freq(j)) / (2.d0 * reb_coher2_freq(j) * dble(tot_interval_reb* reb_array(j)))) / (2.d0 * pi * reb_freq(j)) 
...
!Adam's formula lag
 ...
 end subroutine rebin_lag_freq_obs
